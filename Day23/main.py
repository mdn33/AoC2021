...
def moves_to_hallway(rooms, hallway,room_size):
    ...
    for r, room in enumerate(rooms):
        ...
        for h in range(len(hallway)):
            cost = move_cost(room, hallway, r, h, room_size, to_room=False)
            if cost == INFINITY:
                continue
...
def possible_moves(rooms, hallway,room_size):
    try:
        yield next(moves_to_room(rooms, hallway, room_size)) #take the first one
...
hallway  = (None,) * 7
min_cost = solve(rooms, hallway,2)
